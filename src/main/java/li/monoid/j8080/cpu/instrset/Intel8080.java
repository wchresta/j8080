...
public class Intel8080 implements InstrSet {
    private static final BaseKind[] OP_KINDS = {
            ...
            new URegKind(0xc7, RST, 11, 0),
            new URegKind(0x04, INR, 5, 5),
            new URegKind(0x05, DCR, 5, 5),
            new URegKind(0x06, MVI, 7, 3, 2),
    };

    private static final OpCode[] OP_CODES;

    static {
        OP_CODES = new OpCode[256];
        OpCodes:
        for (int opCode = 0; opCode <= 255; ++opCode) {
            var opCodeByte = Cast.toByte(opCode);
            for (BaseKind opKind : OP_KINDS) {
                if (!opKind.match(opCodeByte)) {
                    continue;
                }
                OP_CODES[opCode] = new OpCode(opCodeByte, opKind.getOpType(), opKind, opKind.fullMnemonic(opCodeByte));
                continue OpCodes;
            }
            OP_CODES[opCode] = new OpCode(opCodeByte, INVALID, new InvalidKind(opCode), "INVALID");
        }
    }
